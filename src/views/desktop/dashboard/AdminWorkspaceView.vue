  ...
  <section class="admin-view">
    <div class="admin-view__shell">
      <StandardHeader
        ...
        @logout="handleLogout"
      />
      <div class="admin-view__breadcrumbs">
        <Breadcrumbs />
      </div>
      ...
      <div v-else-if="section === 'accounts'" class="admin-view__stack">
        <section class="admin-view__grid">
      ...
      <div v-else class="admin-view__stack">
        <div class="admin-view__grid admin-view__grid--profile">
        <article class="admin-view__card">
          <div class="admin-view__card-head">
        ...
        <article class="admin-view__card">
          ...
          <div class="admin-view__list">
            <button class="admin-view__row admin-view__row--interactive" type="button" @click="goToSection('schools')">
              <div><h3>Schools</h3><p>Create, activate, and manage subscription states.</p></div>
              <ArrowRight :size="18" class="admin-view__row-chevron" />
            </button>
            <button class="admin-view__row admin-view__row--interactive" type="button" @click="goToSection('accounts')">
              <div><h3>Campus Admins</h3><p>Reset credentials and control platform access safely.</p></div>
              <ArrowRight :size="18" class="admin-view__row-chevron" />
            </button>
            <button class="admin-view__row admin-view__row--interactive" type="button" @click="goToSection('oversight')">
              <div><h3>Oversight</h3><p>Review audits, notifications, governance requests, and retention.</p></div>
              <ArrowRight :size="18" class="admin-view__row-chevron" />
            </button>
          </div>
        </article>
...
.admin-view__grid--profile{margin-top:24px}
...
.admin-view__row{padding:16px 18px;border-radius:24px;background:var(--color-field-surface);border:none;text-align:left;font-family:inherit;width:100%}
.admin-view__row--interactive{cursor:pointer;display:flex;align-items:center;justify-content:space-between;transition:background .2s ease,transform .2s ease;gap:12px}
.admin-view__row--interactive:hover{background:var(--color-surface);box-shadow:0 8px 24px rgba(15,23,42,.06);transform:translateY(-2px)}
.admin-view__row--interactive:active{transform:translateY(0)}
.admin-view__row-chevron{color:var(--color-primary);opacity:0;transform:translateX(-10px);transition:all .2s ease;flex-shrink:0}
.admin-view__row--interactive:hover .admin-view__row-chevron{opacity:1;transform:translateX(0)}
...
.admin-view__footer{margin-top:16px}
