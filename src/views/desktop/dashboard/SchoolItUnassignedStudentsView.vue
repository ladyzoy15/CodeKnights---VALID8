...
const { state: previewState, deleteStudent: previewDeleteStudent, updateStudentAssignment: previewUpdateAssignment } = useSchoolItPreviewStore()
...
const activeDepartments = computed(() => filterWorkspaceEntitiesBySchool(props.preview ? previewState.departments : departments.value, schoolId.value))
const activePrograms = computed(() => filterWorkspaceEntitiesBySchool(props.preview ? previewState.programs : programs.value, schoolId.value))
const activeUsers = computed(() => filterWorkspaceEntitiesBySchool(props.preview ? previewState.users : users.value, schoolId.value))
...
async function deleteStudent(student) {
  ...
  try {
    if (!props.preview) {
      ...
      previewDeleteStudent(student.id)
    }
  } catch (error) {
...
async function saveStudentAssignment() {
  ...
  try {
    if (!props.preview) {
      ...
      previewUpdateAssignment(selectedStudent.value.id, editDraft.value.departmentId, editDraft.value.programId)
    }
    closeEditSheet()
  } catch (error) {
...
.school-it-unassigned__title { margin: 0; font-size: 26px; font-weight: 800; line-height: 1; letter-spacing: -0.05em; color: var(--color-text-primary); }

.school-it-unassigned__toolbar { display: flex; align-items: stretch; gap: 14px; }
.school-it-unassigned__search-shell { flex: 1; min-width: 0; display: flex; align-items: center; background: var(--color-field-surface); border: 1px solid var(--color-surface-border); border-radius: 999px; padding: 0 16px; height: 56px; }
.school-it-unassigned__search-input { flex: 1; border: none; background: transparent; outline: none; font-size: 14px; font-weight: 500; color: var(--color-text-primary); }
...
.school-it-unassigned__main-card { background: var(--color-surface); border: 1px solid var(--color-surface-border); border-radius: 36px; padding: 32px 24px; display: flex; flex-direction: column; gap: 24px; box-shadow: var(--aura-shadow-soft); }
...
.school-it-unassigned__name { font-size: 14px; font-weight: 600; color: var(--color-text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.school-it-unassigned__action--edit { color: var(--color-text-primary); }
...
.school-it-unassigned__edit-title { margin: 0; font-size: 20px; font-weight: 700; color: var(--color-text-primary); }
.school-it-unassigned__edit-close { border: none; background: transparent; color: var(--color-text-primary); cursor: pointer; }
...
.school-it-unassigned__edit-select { width: 100%; height: 52px; border-radius: 12px; border: 1px solid var(--color-surface-border); background: var(--color-field-surface); padding: 0 16px; font-family: inherit; font-size: 14px; color: var(--color-text-primary); outline: none; }
