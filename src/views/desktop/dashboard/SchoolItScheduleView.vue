    ...
    <div class="school-it-schedule__shell">
      ...
      <div class="school-it-schedule__body">
        ...
        <div class="school-it-schedule__header-row dashboard-enter dashboard-enter--3">
          <button
            class="school-it-schedule__add-btn"
            type="button"
            @click="createNewEvent"
          >
            <span class="school-it-schedule__add-btn-icon">+</span>
            New Event
          </button>
...
const { state: previewState, addEvent: previewAddEvent, updateEvent: previewUpdateEvent, deleteEvent: previewDeleteEvent, updateSettings: previewUpdateSettings } = useSchoolItPreviewStore()
...
async function fetchEvents() {
  ...
  if (props.preview) {
    eventsList.value = previewState.events
...
async function saveSettings() {
  ...
  try {
    ...
    await previewUpdateSettings(payload)
  } catch (error) {
  ...
  try {
    if (props.preview) {
      if (editingEvent.value) {
        previewUpdateEvent(editingEvent.value.id, payload)
        replaceEventInList({ ...editingEvent.value, ...payload })
      } else {
        previewAddEvent(payload)
        fetchEvents()
      }
    } else {
      const token = localStorage.getItem('aura_token') || ''
...
.school-it-schedule__add-btn:hover {
  filter: brightness(1.08);
  transform: translateY(-1px);
}

.school-it-schedule__add-btn:active {
  transform: scale(0.96);
}

.school-it-schedule__add-btn-icon {
  font-size: 20px;
  line-height: 1;
}
