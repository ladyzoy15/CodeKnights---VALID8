...
import { applyTheme, loadTheme, configureThemeForUser, applyLightOverride, removeLightOverride } from '@/config/theme.js'
import {
  getFaceStatus,
  resolveApiBaseUrl,
  saveFaceReference,
...
const introCopy = computed(() => {
  const isStudent = authMeta.value?.roles?.includes('student')
  const workspaceLabel = isStudent ? 'student dashboard' : 'School IT workspace'

  if (mode.value === 'register') {
    return `We will use the same face-scan experience as attendance so future ${workspaceLabel} sign-ins feel familiar.`
  }

  return `We will use the same face-scan experience to confirm this account before your ${workspaceLabel} opens.`
})
...
const captureCopy = computed(() => {
  const isStudent = authMeta.value?.roles?.includes('student')
  const workspaceLabel = isStudent ? 'dashboard' : 'School IT workspace'

  if (statusState.value === 'success') {
    return `Your face verification is complete. We are taking you straight to your ${workspaceLabel}.`
  }
  if (statusState.value === 'submitting') {
  ...
  return mode.value === 'register'
    ...
    : `Use the same school-branded scan flow to unlock your protected ${workspaceLabel}.`
})
...
function describeAntiSpoofReason(reason) {
  switch (reason) {
    ...
    default:
      return 'Face verification is not ready on the backend yet.'
  }
}
...
async function ensurePendingPrivilegedFace() {
  ...
  if (!hasPendingFaceVerification(authMeta.value)) {
...
async function loadFaceStatus() {
  ...
  try {
    ...
    setPendingFaceError(error?.message || 'Unable to load face verification status.', false)
  }
}
