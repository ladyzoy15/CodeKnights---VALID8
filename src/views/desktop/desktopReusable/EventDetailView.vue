  ...
  <div class="event-detail">
    ...
    <section v-if="event" class="detail-body">
      ...
      <div class="title-block dashboard-enter dashboard-enter--2">
        ...
        <div v-if="activeUser?.roles?.includes('student') || activeUser?.roles?.includes('Student')" class="mt-4">
          <button 
            v-if="event?.status === 'upcoming' && !hasSubmittedExcuse" 
          ...
          <div v-else-if="hasSubmittedExcuse" class="inline-flex items-center space-x-2 bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700">
            <span class="text-sm text-gray-600 dark:text-gray-300">Excuse Letter:</span>
            <span :class="{
              'px-2 py-0.5 rounded text-xs font-medium': true,
              'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400': excuseStatus === 'Pending',
              'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400': excuseStatus === 'Approved',
              'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400': excuseStatus === 'Rejected'
            }">{{ excuseStatus }}</span>
          </div>
        </div>
      </div>
    ...
    <ExcuseLetterSubmitModal
      v-model="showExcuseModal"
      :event="event"
      @submitted="excuseLetters.fetchStudentLetters()"
    />
  </div>
</template>
...
import { useExcuseLetters } from '@/composables/useExcuseLetters.js'
...
const eventName = computed(() => event.value?.name || 'Loading Event...')

const excuseLetters = useExcuseLetters()
const showExcuseModal = ref(false)

const hasSubmittedExcuse = computed(() => {
  return excuseLetters.letters.value.some(l => Number(l.eventId) === Number(eventId.value))
})

const excuseStatus = computed(() => {
  const letter = excuseLetters.letters.value.find(l => Number(l.eventId) === Number(eventId.value))
...
onMounted(async () => {
  ...
  // Also fetch excuse letters to determine button state
  if (activeUser.value?.roles?.includes('student') || activeUser.value?.roles?.includes('Student')) {
    await excuseLetters.fetchStudentLetters()
  }
})
