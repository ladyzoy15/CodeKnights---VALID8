    ...
    <div class="school-it-settings__shell">
      ...
      <div class="school-it-settings__body">
        ...
        <section class="school-it-settings__color-grid dashboard-enter dashboard-enter--3">
          ...
          <section class="school-it-settings__face-card dashboard-enter dashboard-enter--4">
          <div class="school-it-settings__face-header">
            <ShieldCheck :size="20" class="school-it-settings__face-icon" />
            <h3 class="school-it-settings__face-title">Face Recognition</h3>
          </div>

          <div class="school-it-settings__toggles">
            <div class="school-it-settings__toggle-row">
              <div class="school-it-settings__toggle-copy">
                <p class="school-it-settings__toggle-label">Privileged Verification</p>
                <p class="school-it-settings__toggle-hint">Require face scan for Campus Admins.</p>
              </div>
              <button
                class="school-it-settings__switch"
                :class="{ 'school-it-settings__switch--active': draft.privileged_face_verification_enabled }"
                type="button"
                :disabled="isSavingFace || controlsDisabled"
                @click="toggleFaceSetting('privileged_face_verification_enabled')"
              >
                <span class="school-it-settings__switch-thumb" />
              </button>
            </div>

            <div class="school-it-settings__toggle-row">
              <div class="school-it-settings__toggle-copy">
                <p class="school-it-settings__toggle-label">Attendance Recognition</p>
                <p class="school-it-settings__toggle-hint">Use face recognition for student attendance.</p>
              </div>
              <button
                class="school-it-settings__switch"
                :class="{ 'school-it-settings__switch--active': draft.attendance_face_recognition_enabled }"
                type="button"
                :disabled="isSavingFace || controlsDisabled"
                @click="toggleFaceSetting('attendance_face_recognition_enabled')"
              >
                <span class="school-it-settings__switch-thumb" />
              </button>
            </div>
            ...
            <div class="school-it-settings__toggle-row">
              <div class="school-it-settings__toggle-copy">
                <p class="school-it-settings__toggle-label">Mandatory Registration</p>
                <p class="school-it-settings__toggle-hint">Force face registration on first login.</p>
              </div>
              <button
                class="school-it-settings__switch"
                :class="{ 'school-it-settings__switch--active': draft.first_time_face_registration_required }"
                type="button"
                :disabled="isSavingFace || controlsDisabled"
                @click="toggleFaceSetting('first_time_face_registration_required')"
              >
                <span class="school-it-settings__switch-thumb" />
              </button>
            </div>
          ...
          class="school-it-settings__feedback dashboard-enter dashboard-enter--5"
          :class="{
          ...
          import { LoaderCircle, Upload, ShieldCheck } from 'lucide-vue-next'
          ...
          const isSavingFace = computed(() => pendingField.value === 'face')
          ...
          draft.privileged_face_verification_enabled = value.privileged_face_verification_enabled !== false
          draft.attendance_face_recognition_enabled = value.attendance_face_recognition_enabled !== false
          draft.first_time_face_registration_required = value.first_time_face_registration_required !== false
          },
          ...
          async function toggleFaceSetting(key) {
          if (controlsDisabled.value || pendingField.value) return

          const previousValue = draft[key]
          draft[key] = !previousValue

          if (isPreviewWorkspace.value) {
          previewSettings[key] = draft[key]
          pushFeedback('success', 'Face setting updated in preview.')
          return
          }

          const saved = await persistBranding('face')
          if (!saved) {
          draft[key] = previousValue
          }
          }
          ...
          privileged_face_verification_enabled: draft.privileged_face_verification_enabled,
          attendance_face_recognition_enabled: draft.attendance_face_recognition_enabled,
          first_time_face_registration_required: draft.first_time_face_registration_required,
          }, logoFile)
          ...
          pushFeedback('success', kind === 'logo' ? 'University logo updated.' : kind === 'face' ? 'Face recognition settings updated.' : 'Branding updated.')
          return true
          } catch (error) {
          pushFeedback('error', error?.message || 'Unable to update school settings right now.')
          return false
          } finally {
          pendingField.value = ''
          }
          ...
          privileged_face_verification_enabled: value?.privileged_face_verification_enabled !== false,
          attendance_face_recognition_enabled: value?.attendance_face_recognition_enabled !== false,
          first_time_face_registration_required: value?.first_time_face_registration_required !== false,
          }
          }
          ...
          .school-it-settings__preview-card,.school-it-settings__logo-card,.school-it-settings__color-card,.school-it-settings__face-card{background:var(--color-surface);border-radius:32px;box-shadow:0 18px 40px rgba(15,23,42,.04)}
          ...
          .school-it-settings__face-card{padding:20px 18px;display:flex;flex-direction:column;gap:20px}
          .school-it-settings__face-header{display:flex;align-items:center;gap:10px;color:var(--color-text-primary)}
          .school-it-settings__face-icon{color:var(--color-primary)}
          .school-it-settings__face-title{margin:0;font-size:16px;font-weight:700;letter-spacing:-.01em}
          .school-it-settings__toggles{display:flex;flex-direction:column;gap:16px}
          .school-it-settings__toggle-row{display:flex;align-items:center;justify-content:space-between;gap:20px}
          .school-it-settings__toggle-copy{display:flex;flex-direction:column;gap:2px}
          .school-it-settings__toggle-label{margin:0;font-size:14px;font-weight:600;color:var(--color-text-primary)}
          .school-it-settings__toggle-hint{margin:0;font-size:12px;color:var(--color-text-secondary);line-height:1.3}
          .school-it-settings__switch{position:relative;width:44px;height:24px;border:none;border-radius:999px;background:#E2E2DE;padding:0;transition:background-color .2s ease,transform .16s ease;cursor:pointer}
          .school-it-settings__switch--active{background:var(--color-primary)}
          .school-it-settings__switch:disabled{opacity:.64;cursor:not-allowed}
          .school-it-settings__switch:not(:disabled):active{transform:scale(.94)}
          .school-it-settings__switch-thumb{position:absolute;top:3px;left:3px;width:18px;height:18px;border-radius:999px;background:#FFFFFF;box-shadow:0 2px 4px rgba(0,0,0,.08);transition:transform .24s cubic-bezier(.22,1,.36,1)}
          .school-it-settings__switch--active .school-it-settings__switch-thumb{transform:translateX(20px)}
          ...
          .school-it-settings__face-card{padding:18px 16px;border-radius:28px}
          }
          ...
          .school-it-settings__toggle-label{font-size:13px}
          .school-it-settings__toggle-hint{font-size:11px}
          }
          </style>
