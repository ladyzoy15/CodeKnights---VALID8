...
## 2026-04-16 - Standardize backend system-name defaults and notification copy to NEXUS

### Purpose

Align backend-generated messaging with the current product name by replacing remaining `VALID8/Valid8` runtime defaults with `NEXUS`.
...
- changed default email sender display name to `NEXUS Notifications` when `EMAIL_FROM_NAME`/`SMTP_FROM_NAME` are unset
- changed email use-case fallback `system_name` to `NEXUS` when no school-scoped name is provided
- updated default `/api/notifications/test` subject/body copy to NEXUS branding
- updated missed/low/reminder notification footers and sanctions-task signatures to `NEXUS`
- updated email-transport connectivity-test default subject/body/html copy to NEXUS
- updated operator smoke-test script defaults to NEXUS wording
- updated OAuth token helper script description text to NEXUS wording
...
- no request/response schema changes
- runtime behavior change:
  - default notification/email text generated by backend now uses `NEXUS` branding

### Migration impact

- no database migrations required
- configuration default changed:
  - `EMAIL_FROM_NAME` fallback is now `NEXUS Notifications`
...
2. Run backend transport smoke test command:
   ...
   - verify the default subject starts with `NEXUS email transport smoke test`
3. Call `POST /api/notifications/test` without a custom `message` and verify the generated default text/subject uses `NEXUS`.
...
Enable realistic manual testing of the UI + NEXUS AI assistant across multiple schools, roles, and governance permission codes.
